...
<details>
<summary>Q5. 反過來說，什麼情況「不該」用成員變數，該用區域變數就好？</summary>

A5. 判斷準則：**這個值需不需要跨函式呼叫、或跨遞迴層次持續存在？** 不需要的話，宣告成成員變數只是多餘的複雜度，還可能造成誤會（讓人以為這個值真的需要跨呼叫共用）。

常見的誤用：一個輔助函式（例如只計算一次就結束、不遞迴、不會被同一物件重複呼叫來累積狀態的函式）裡用到的中間變數，被習慣性地宣告成成員變數，但其實這些值只在**這一次呼叫內部**用完就丟，跟 Q3 的 Implement Queue using Stacks（`sIn`/`sOut` 真的需要跨 `push`/`pop`/`peek` 持續存在）是相反的情境。

- K Closest Points to Origin：`pSumOfSquares` 裡的 `x`、`y` 只在該次函式呼叫內使用，宣告成成員變數是多餘的，改成區域變數更合適
- 3Sum：一開始把 `pinIdx`、`left`、`right`、`rltSum` 都設計成成員變數、外加一個獨立的 `formula()` 輔助函式，但這些值不需要跨呼叫保留狀態，改成全部宣告在 `threeSum` 函式內部的區域變數、不拆函式，程式碼更精簡
...
| 15 | 3Sum：反例——一開始把不需要跨呼叫共用的變數設計成成員變數、外加獨立輔助函式，參考解法示範全部用區域變數、單一函式就能完成，見 Q5 |
| 133 | Clone Graph：正當使用——`unordered_map<Node*,Node*> mp` 需要在 `cloneGraph` 遞迴呼叫自己的所有層之間共用（記錄已複製的節點），必須是成員變數，宣告成區域變數會每層重置成空 |
