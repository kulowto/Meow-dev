...
| `專案/_pipeline_ComposeAssetsToVideo/` | 專案材料 | 影片製作流水線系統：工具鏈架構、模組索引、流程驗證邏輯 |
| `專案/轉職程式設計師_三個月計畫/` | 專案材料 | 三個月轉職規劃：LeetCode 刷題、Spaced Repetition 學習記錄、UMPIRE 面試表達訓練 |
...
### 專案/_pipeline_ComposeAssetsToVideo/
影片內容製作流水線系統開發工作區。本專案負責串接各獨立能力模組、定義執行順序與人工審核節點，不重複實作各模組功能。
→ 詳見 `専案/_pipeline_ComposeAssetsToVideo/MENU.md`

### 專案/轉職程式設計師_三個月計畫/
三個月轉職程式設計師的學習規劃，由 `/inquiry` 討論沉澱而來，因規劃已展開為獨立專案，故獨立成資料夾而非留在 `inquiry_model/sessions/` 通用區。

| 文件 | 內容 |
|------|------|
| `2026-08-05_三個月英文+程式能力提升規劃.md` | Inquiry 討論彙整：刷題範疇、SRS 記錄系統設計、UMPIRE 面試訓練三支柱 |
| `2026-08-05_三個月英文+程式能力提升規劃.json` | 上述討論的結構化資料版本 |
| `總覽表.csv` | （參考用，已被合併版 Google Sheet 取代）SM-2 演算法總覽表初版範本 |
| `複習紀錄表.csv` | （參考用，已被合併版 Google Sheet 取代）複習紀錄表初版範本 |
| `Sheet匯入設定說明.md` | 目前使用中的 Google Sheet 連結、操作流程、SM-2 手動更新公式說明 |
| `(AI_Read) 刷題詳細記錄使用規範.md` | Review 三維度標準、檔案累加規則、盲點彙總登記標準 | AI讀 |
| `(AI_Read) 知識庫重整規劃書.md` | 知識庫壓縮重整的思考與執行方式（合併卡片/導覽/連結稽核），供未來 AI 接手 | AI讀 |
| `刷題詳細記錄/_範本.md` | 單題深度記錄範本：語法/邏輯複雜度/演算法盲點三維度，同題複習往下累加 |
| `刷題詳細記錄/導覽.md` | 32 題總表：題號/LC#/狀態/方法論重點/語法重點/犯過的錯誤，篩選同類補強用 |
| `跨題目盲點彙總.md` | 跨題目重複盲點彙總表，判斷整體弱項用 |
| `語法概念卡/_範本.md` | 跨題複用的語法/容器/技巧知識卡，`<details>` 自我測驗格式 |
| `語法概念卡/導覽.md` | 19 張概念卡索引：軸一方法論/軸二語法分類 + 一句話摘要 + 判斷流程 |
